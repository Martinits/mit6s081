...
// mappages for proc's kpgt, skip if pte already exists 
int
...
// uvmunmap for proc's kernel pagetable, NOT freeing any physical page
void
...
void
ukvmdealloc(pagetable_t kpgt, uint64 oldsz, uint64 newsz)
{
  if(newsz >= oldsz)
    return;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    ukvmunmap(kpgt, PGROUNDUP(newsz), npages);
  }
}
...
// copy a range of user vm pte to a proc's kernel pagetable
// DONOT alloc any physical pages
int
ukvmcopy(pagetable_t upgt, pagetable_t kpgt, uint64 from, uint64 to)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = from; i < to; i += PGSIZE){
    if((pte = walk(upgt, i, 0)) == 0)
      panic("ukvmcopy: pte should exist but not");
    if((*pte & PTE_V) == 0){
      panic("ukvmcopy: page not present(valid)");
    }
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte) & (~PTE_U);
    if(kmappages(kpgt, i, PGSIZE, pa, flags) != 0){
      goto err;
    }
  }
  return 0;
...
// implementation of vmprint
static void
vmprint_recur(pagetable_t pagetable, int predots)
{
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if(! (pte & PTE_V)) continue;

    for(int j = 0; j < predots-1; j++) printf(".. ");
    uint64 child = PTE2PA(pte);
    printf("..%d: pte %p pa %p\n", i, pte, child);
    if((pte & (PTE_R | PTE_W | PTE_X)) == 0){
      vmprint_recur((pagetable_t)child, predots+1);
    }
  }
}

// print specific pagetable
void
